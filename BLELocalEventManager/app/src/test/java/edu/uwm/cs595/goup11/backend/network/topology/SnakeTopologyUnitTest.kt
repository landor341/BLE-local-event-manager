...
class SnakeTopologyUnitTest {
    ...
    @Test
    fun `shouldAcceptConnection accepts when below max`() = runBlocking {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
        assertTrue(topo.shouldAcceptConnection(tc.ctx, "B", aName("B")))
    }

    @Test
    fun `shouldAcceptConnection rejects when at max`() = runBlocking {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
    ...
    @Test
    fun `shouldAcceptConnection accepts again after disconnect frees a slot`() = runBlocking {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `shouldAcceptConnection rejects after single peer at max 1`() = runBlocking {
        val topo = makeTopo(max = 1);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `onPeerConnected increments peer count`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `onPeerConnected duplicate does not double-count`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `onPeerDisconnected decrements peer count`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `onPeerDisconnected unknown id is no-op`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
    ...
    @Test
    fun `shouldAdvertise true when no peers`() {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
    ...
    @Test
    fun `shouldAdvertise false when at max`() {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `shouldAdvertise true again after peer leaves`() {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `TEXT_MESSAGE is not consumed`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
    ...
    @Test
    fun `direct peer is returned as next hop`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `unknown destination floods to all non-sender peers`() {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
    ...
    @Test
    fun `flood never includes sender`() {
        val topo = makeTopo(max = 3);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `empty hop list when no peers`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
    ...
    @Test
    fun `empty hop list after last peer disconnects`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerDisconnected(tc.ctx, "B")
    ...
    @Test
    fun `silent peer is evicted after timeout`() = runBlocking {
    ...
    @Test
    fun `two silent peers both evicted`() = runBlocking {
    ...
    @Test
    fun `eviction triggers re-advertisement`() = runBlocking {
    ...
    @Test
    fun `keepalive sends PINGs to peers`() = runBlocking {
    ...
    @Test
    fun `peer sending PONGs is not evicted`() = runBlocking {
        val topo = makeTopo(keepaliveMs = 50, timeoutMs = 120)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `only silent peer evicted when one responds`() = runBlocking {
    ...
    @Test
    fun `discovery loop calls startAdvertising`() = runBlocking {
    ...
    @Test
    fun `discovery loop stops when max peers reached`() = runBlocking {
        val topo = makeTopo(max = 2, discoveryMs = 50)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        delay(100)
        ...
        assertEquals(
            false, tc.advLog.lastOrNull()?.first,
    ...
    @Test
    fun `discovery loop restarts after peer disconnects`() = runBlocking {
        val topo = makeTopo(max = 2, discoveryMs = 50)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `topology connects to discovered peer with matching topology code`() = runBlocking {
        val topo = makeTopo(max = 2, discoveryMs = 50)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
    ...
    @Test
    fun `topology ignores discovered peer with wrong topology code`() = runBlocking {
        val topo = makeTopo(max = 2, discoveryMs = 50)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        delay(50)
    ...
    @Test
    fun `topology does not connect when already at max peers`() = runBlocking {
    ...
    @Test
    fun `topology does not connect to known chain member`() = runBlocking {
        val topo = makeTopo(max = 2, discoveryMs = 50)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `peer connect-disconnect cycle 5 times stays correct`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        repeat(5) { i ->
            topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `simultaneous disconnect of both peers reaches zero`() {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `rapid random connects and disconnects never corrupt count`() {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
    ...
    @Test
    fun `manual disconnect before keepalive tick no double eviction`() = runBlocking {
        val topo = makeTopo(keepaliveMs = 50, timeoutMs = 120)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `evicted peer can reconnect`() = runBlocking {
        val topo = makeTopo(keepaliveMs = 50, timeoutMs = 120)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `rapid connection storm at max rejected every time`() = runBlocking {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `middle node dropout triggers re-discovery on both ends`() = runBlocking {
        ...
        delay(200)
        assertTrue(
            tcA.advLog.any { it.first },
            "A should be advertising again after B dropped"
        )
        assertTrue(
            tcC.advLog.any { it.first },
    ...
    @Test
    fun `new peer fills slot left by dropped middle node`() {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `message routes through middle node`() {
        val topoA = makeTopo(max = 2);
        val tcA = makeCtx("A")
        val topoB = makeTopo(max = 2);
        val tcB = makeCtx("B")
        topoA.onPeerConnected(tcA.ctx, "B", aName("B"))
    ...
    @Test
    fun `message never bounces back to sender`() {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("B")
    ...
    @Test
    fun `isolated node has no route after chain break`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerDisconnected(tc.ctx, "B")
...
// =========================================================================
    ...
    @Test
    fun `third peer already in chain is rejected to prevent ring`() = runBlocking {
    ...
    @Test
    fun `self-connection is always rejected`() = runBlocking {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("A")
        topo.start(tc.ctx)
    ...
    @Test
    fun `chain member set resets after peer disconnects`() = runBlocking {
    ...
    @Test
    fun `HELLO message merges chain members and propagates to other peers`() = runBlocking {
        val topo = makeTopo(max = 2);
        val tc = makeCtx("B")
        topo.start(tc.ctx)
        ...
        assertNotNull(
            helloWithD,
            "B should forward a HELLO to C that includes newly learned member D"
        )
