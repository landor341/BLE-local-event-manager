...
class MeshTopologyUnitTest {
    ...
    @Test
    fun `accepts connection when below max`() = runBlocking {
        val topo = makeTopo(max = 4, target = 2);
        val tc = makeCtx("A")
        assertTrue(topo.shouldAcceptConnection(tc.ctx, "B", aName("B")))
    }

    @Test
    fun `accepts connection when between target and max`() = runBlocking {
    ...
    @Test
    fun `rejects connection at maxPeerCount`() = runBlocking {
        val topo = makeTopo(max = 3, target = 2);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `accepts again after disconnect drops below max`() = runBlocking {
        val topo = makeTopo(max = 2, target = 1);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `rapid storm of connections rejected when at max`() = runBlocking {
        val topo = makeTopo(max = 2, target = 1);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `onPeerConnected increments count`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `onPeerDisconnected decrements count`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `multiple peers up to max all tracked`() {
        val topo = makeTopo(max = 4, target = 2);
        val tc = makeCtx("A")
    ...
    @Test
    fun `shouldAdvertise true when empty`() {
        val topo = makeTopo(max = 4, target = 2);
        val tc = makeCtx("A")
    ...
    @Test
    fun `shouldAdvertise true when at target but below max`() {
    ...
    @Test
    fun `shouldAdvertise false when at maxPeerCount`() {
        val topo = makeTopo(max = 3, target = 2);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `shouldAdvertise true again after peer leaves max`() {
        val topo = makeTopo(max = 2, target = 1);
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerConnected(tc.ctx, "C", aName("C"))
    ...
    @Test
    fun `PING sends PONG back to sender`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `direct peer returned as next hop`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `unknown destination floods to all peers`() {
        val topo = makeTopo(max = 4, target = 2);
        val tc = makeCtx("A")
    ...
    @Test
    fun `empty hop list after all peers disconnect`() {
        val topo = makeTopo();
        val tc = makeCtx("A")
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
        topo.onPeerDisconnected(tc.ctx, "B")
    ...
    @Test
    fun `direct delivery preferred over flood even with many peers`() {
        val topo = makeTopo(max = 6, target = 3);
        val tc = makeCtx("A")
    ...
    @Test
    fun `below target triggers advertising and scan`() = runBlocking {
    ...
    @Test
    fun `at target but below max stops scan but keeps advertising`() = runBlocking {
    ...
    @Test
    fun `at max stops both advertising and scan`() = runBlocking {
    ...
    @Test
    fun `dropping from max back to target zone resumes advertising only`() = runBlocking {
    ...
    @Test
    fun `dropping below target from target zone resumes full discovery`() = runBlocking {
        val topo = makeTopo(max = 4, target = 2, discoveryMs = 50)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `silent peer evicted after timeout`() = runBlocking {
    ...
    @Test
    fun `all silent peers evicted`() = runBlocking {
    ...
    @Test
    fun `eviction triggers re-discovery`() = runBlocking {
    ...
    @Test
    fun `keepalive sends PINGs to all connected peers`() = runBlocking {
    ...
    @Test
    fun `manual disconnect before keepalive tick causes no double eviction`() = runBlocking {
        val topo = makeTopo(keepaliveMs = 50, timeoutMs = 120)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `only silent peer evicted when others respond`() = runBlocking {
    ...
    @Test
    fun `discovery loop starts when below target`() = runBlocking {
        val topo = makeTopo(max = 4, target = 2, discoveryMs = 50)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        delay(200)
    ...
    @Test
    fun `discovery loop stops scanning at target not max`() = runBlocking {
    ...
    @Test
    fun `advertising stops when max is reached`() = runBlocking {
    ...
    @Test
    fun `discovery restarts after peer count drops below target`() = runBlocking {
        val topo = makeTopo(max = 4, target = 2, discoveryMs = 50)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
        topo.onPeerConnected(tc.ctx, "B", aName("B"))
    ...
    @Test
    fun `all peers disconnect simultaneously — count reaches zero`() {
        val topo = makeTopo(max = 4, target = 2);
        val tc = makeCtx("A")
        listOf("B", "C", "D").forEach { topo.onPeerConnected(tc.ctx, it, aName(it)) }
    ...
    @Test
    fun `rapid random connects and disconnects never exceed max`() {
        val topo = makeTopo(max = 3, target = 2);
        val tc = makeCtx("A")
    ...
    @Test
    fun `peer evicted by keepalive then reconnects and gets evicted again`() = runBlocking {
        val topo = makeTopo(keepaliveMs = 50, timeoutMs = 120)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
    ...
    @Test
    fun `partial reconnect after mass disconnect`() = runBlocking {
        val topo = makeTopo(max = 4, target = 2, keepaliveMs = 50, timeoutMs = 10_000)
        val tc = makeCtx("A")
        topo.start(tc.ctx)
    ...
    @Test
    fun `all direct peers get direct delivery`() {
        val topo = makeTopo(max = 4, target = 2);
        val tc = makeCtx("A")
        listOf("B", "C", "D").forEach { topo.onPeerConnected(tc.ctx, it, aName(it)) }
        assertEquals(listOf("B"), topo.resolveNextHop(tc.ctx, msg("B", "A")))
    ...
    @Test
    fun `message to unknown floods to all peers in full mesh`() {
    ...
    @Test
    fun `flood in mesh never includes message sender`() {
        val topo = makeTopo(max = 6, target = 3);
        val tc = makeCtx("A")
        listOf("B", "C", "D", "E").forEach { topo.onPeerConnected(tc.ctx, it, aName(it)) }
    ...
    @Test
    fun `isolated node after full disconnect has no route`() {
        val topo = makeTopo(max = 4, target = 2);
        val tc = makeCtx("A")
        listOf("B", "C").forEach {
    ...
    @Test
    fun `two separate nodes each see their own peers`() {
        val topoA = makeTopo(max = 4, target = 2);
        val tcA = makeCtx("A")
        val topoB = makeTopo(max = 4, target = 2);
        val tcB = makeCtx("B")
